...
export const getLanguageCode = (key: string): string => {
    return LanguageList.filter(p => p.key.toLowerCase() === key.toLowerCase())[0].code;
};
export const getLanguageKey = (code: string): string => {
    return LanguageList.filter(p => p.code.toLowerCase() === code.toLowerCase())[0].key;
};
...
export const isArabicCurrentLanguage = (): boolean => {
    // @ts-ignore
    return Cookies.get(process.env.REACT_APP_languageStorageKey) ? CookieGet(process.env.REACT_APP_languageStorageKey) == "ar-AE" ? true : false : false;
};